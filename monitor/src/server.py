...
class UDPServer:
    def __init__(self):
        rospy.init_node("UDP_Server_node",log_level=rospy.INFO)
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.sock.bind(('127.0.0.1', 9999))

        self.pub_current_pose_ = rospy.Publisher("/ndt/current_pose",PoseStamped,queue_size=1)
    ...
    def run(self):
        ...
        while True:
            ...
            if base_msg.msg_type == protoc_msg_pb2.current_pose:
                ...
                self.pub_current_pose_.publish(current_pose_msg)
                print "---------------"
                print "msg.header: "
